...
class CourseSelectRepositoryImpl implements CourseSelectRepository {
  ...
  Future<Either<Failure, CourseSchedule>> getCourseSchedule() async {
    try {
      ...
      final courseSchedule = CourseSelectParser.parseSchedule(
          responseBody); // Parse the response into CourseSchedule.
      return Right(
          courseSchedule.toEntity()); // Return the parsed course schedule.
    } on ServerException catch (e) {
      return Left(
          ServerFailure(message: e.message)); // Return failure on server error.
  ...
  Future<Either<Failure, Semester>> getCurrentSemester() async {
    try {
      ...
      final semester = CourseSelectParser.parseCurrentSemester(
          responseBody); // Parse current semester information.
  ...
  Future<Either<Failure, List<Semester>>> getSemesterList() async {
    try {
      ...
      final responseBody =
          await crsselCrawler.getCourseTablePage(); // Fetch course table page.
      final semesterList = CourseSelectParser.parseSemesterList(
          responseBody); // Parse the list of semesters.
      return Right(semesterList
          .map((e) => e.toEntity())
          .toList()); // Return the list of semesters.
    } on ServerException catch (e) {
      return Left(
          ServerFailure(message: e.message)); // Return failure on server error.
    } on SessionExpiredException catch (e) {
